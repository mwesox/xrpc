...
describe('TypeScript Client E2E', () => {
  let testDir: string;
  ...
  test('generates TypeScript client, starts server, and handles requests', async () => {
    // Create temporary directory
    const timestamp = Date.now();
    const random = randomBytes(4).toString('hex');
    testDir = join(process.cwd(), 'tmp', `e2e-ts-client-test-${timestamp}-${random}`);
    ...
    // Install zod as dependency since the generated code imports it
