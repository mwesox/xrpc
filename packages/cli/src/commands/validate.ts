...
export async function validateCommand(
  ...
  if (!filePath && prompt) {
    ...
    throw new Error(
      "File path is required. Use: xrpc validate <file> or run in interactive mode.",
    );
  }
  ...
  const validateSpinner = createSpinner
    ? createSpinner("Validating contract...")
    : null;
  ...
  try {
    ...
    if (syntaxErrors.length > 0) {
      ...
      for (const error of syntaxErrors) {
        console.error(
          `  ${formatError(`Line ${error.line}: ${error.message}`)}`,
        );
      }
      process.exit(1);
    }
...
async function validateStructure(filePath: string): Promise<string[]> {
  ...
  try {
    ...
    if (!contract.endpoints || contract.endpoints.length === 0) {
      errors.push(
        "No endpoints found. Router must contain at least one endpoint.",
      );
      return errors;
    }
    ...
    for (const endpoint of contract.endpoints) {
      ...
      if (
        ...
        errors.push(
          `Endpoint "${endpoint.fullName}" has invalid type: ${endpoint.type}. Must be "query" or "mutation".`,
        );
      }
      if (!endpoint.input) {
      ...
      if (!endpoint.output) {
        errors.push(
          `Endpoint "${endpoint.fullName}" is missing output schema.`,
        );
      }
    }
    ...
    if (error instanceof Error) {
      if (error.message.includes("No router export")) {
        errors.push(
          "Contract must export a router. Example: export const router = createRouter({ ... });",
        );
      } else {
