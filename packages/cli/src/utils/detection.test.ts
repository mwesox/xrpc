...
import { detectProject, detectApps, detectExistingContracts } from './detection';
import { generateTomlTemplate, generatePackageJsonTemplate } from './templates';
import { join } from 'node:path';

const EXAMPLE_DIR = join(import.meta.dir, '../../../../examples/x-rpc-todo-app');
...
describe('detection utilities', () => {
  test('detects turborepo monorepo with apps', async () => {
    ...
    expect(webApp?.suggestedTarget).toBe('react-client');
...
describe('template generation', () => {
  test('generates valid TOML config', () => {
    const toml = generateTomlTemplate({
      ...
      targets: [
        ...
        { name: 'react-client', outputPath: 'apps/web' },
      ],
    });
    ...
    expect(parsed['react-client']).toBe('apps/web');
  });
  ...
  test('generates valid package.json', () => {
    ...
    expect(parsed.dependencies['@xrpckit/schema']).toBeDefined();
