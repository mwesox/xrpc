...
- `@xrpckit/target-ts-server` - TypeScript server
- `@xrpckit/target-kotlin-spring-boot-server` - Kotlin Spring Boot server
- `@xrpckit/target-swift-client` - Swift URLSession client

CLI target names: `go-server`, `ts-client`, `ts-server`, `kotlin-spring-boot-server`, `swift-client`.
...
- [Configuration](../configuration/) - Configure code generation
