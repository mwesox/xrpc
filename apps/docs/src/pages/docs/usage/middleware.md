...
> **Note**: TypeScript middleware hooks are planned for the `ts-server` target. Go middleware is available today.
...
**Planned (`ts-server`)**: The API below is a draft and may change. It illustrates the intended flow.
...
Go access uses type assertions to handle missing context gracefully. TypeScript context typing is planned for `ts-server`.
...
- [TypeScript Server](../typescript-server/) - TypeScript server setup
- [Go Server](../go-server/) - Full Go server setup
- [API Contract](../api-contract/) - Define your schema
